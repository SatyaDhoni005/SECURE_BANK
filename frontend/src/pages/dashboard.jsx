...
import { Landmark, LogOut, AlertCircle, User, Settings as SettingsIcon, Bell, X } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchAccountDetails = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Failed to sync transactions for dynamic notifications:", e);
      }
    } catch (err) {
      setError(err.message || 'Failed to sync with the secure banking mainframe.');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      try {
        ...
        const apiBaseHost = (import.meta.env.VITE_API_URL || 'http://localhost:8000/api')
          .replace('http://', '')
          .replace('https://', '')
          .split('/')[0];
        ...
        socket.onopen = () => {
          console.log("Secure bank mainframe WebSockets telemetry link active.");
        };
        ...
        socket.onclose = (event) => {
          console.log("WebSocket telemetry link disconnected. Reason code:", event.code);
        ...
        console.error("Failed to initialize WebSocket telemetry connection:", err);
      }
    };
  ...
  useEffect(() => {
    if (data) {
      ...
      const readIds = JSON.parse(localStorage.getItem(`read_notifications_${email}`) || '[]');
      ...
      if (!data.pin_created) {
        list.push({
          ...
          message: 'Your Transaction PIN has not been configured. Setup a PIN to secure balance access.',
          time: 'Just now',
          timestamp: Date.now()
        });
      } else {
        list.push({
          ...
          message: 'Transaction PIN configured. Multi-factor vault security is active.',
          time: 'Active',
      ...
      list.push({
        ...
        message: 'Security KYC compliance verification checks completed successfully.',
        time: 'System',
  ...
  const markAllAsRead = () => {
    if (data) {
      ...
      localStorage.setItem(`read_notifications_${email}`, JSON.stringify(allIds));
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ color: 'white', fontSize: '1.5rem', marginBottom: '0.75rem', fontWeight: 800 }}>
            Session Synced Error
          </h2>
          <p style={{ color: 'rgba(255, 255, 255, 0.65)', fontSize: '0.9rem', lineHeight: '1.5', marginBottom: '2rem' }}>
            {error}
          </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '0.75rem' : '1.5rem' }}>
          {!isMobile && (
            <div style={{ textAlign: 'right', display: 'block' }}>
              ...
              <div style={{ fontSize: '0.85rem', fontWeight: 700 }}>{data.name}</div>
            </div>
          )}
          ...
          <div style={{ position: 'relative' }} ref={notificationRef}>
            <button
              onClick={() => isMobile ? setShowDrawer(true) : handleToggleNotifications()}
              style={{
                width: '40px',
                height: '40px',
                borderRadius: '50%',
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#0A2540' }}>Security & Ledger Alerts</span>
                {unreadCount > 0 && (
                  <button
              ...
              <div style={{ borderBottom: '1px solid var(--border-color)', marginBottom: '0.75rem' }}></div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '320px', overflowY: 'auto' }}>
                {notifications.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '1.5rem 0', color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                  ...
                  notifications.map((notif) => {
                    ...
                    return (
                      <div key={notif.id} style={{
                        ...
                        border: notif.read ? '1px solid transparent' : '1px solid rgba(10, 37, 64, 0.05)',
                        transition: 'all 0.2s',
                        alignItems: 'center'
                      }}>
                        <div style={{
                          ...
                          height: '32px',
                          borderRadius: '50%',
                          backgroundColor: notif.type === 'credit' || notif.type === 'success' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                          flexShrink: 0
                        }}>
                          {icon}
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: '#0A2540' }}>{notif.title}</span>
                            {!notif.read && (
                              <span style={{
                                width: '6px',
                                height: '6px',
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', lineHeight: '1.3', marginTop: '0.15rem' }}>
                            {notif.message}
                          </div>
                          <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginTop: '0.25rem', fontWeight: 600 }}>
                            {notif.time}
                          </div>
                        </div>
                      </div>
          ...
          <div style={{ position: 'relative' }} ref={dropdownRef}>
            <button
              onClick={() => isMobile ? setShowDrawer(true) : setShowDropdown(!showDropdown)}
              style={{
                width: '42px',
                height: '42px',
                borderRadius: '50%',
        ...
        <AnimatedStagger type="slide-up" interval={80} duration={900}>
          <div style={{ marginBottom: '2.5rem' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>
              Your secure vault portal is synchronized. Last verified: {new Date().toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
          </div>
          ...
          {data?.pin_created === false && (
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                <h3 style={{ margin: '0 0 0.5rem 0', color: '#0A2540', fontSize: '1.15rem', fontWeight: 800 }}>
                  Security Alert
                </h3>
                <p style={{ margin: '0 0 0.75rem 0', color: 'rgba(10, 37, 64, 0.85)', fontSize: '0.95rem', fontWeight: 600, lineHeight: '1.4' }}>
                  Your Transaction PIN has not been configured.
                </p>
                <div style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.25rem' }}>
                  Create a PIN to unlock:
                  <ul style={{ margin: '0.4rem 0 0 1.25rem', padding: 0, listStyleType: 'disc', display: 'flex', flexDirection: 'column', gap: '0.2rem', fontWeight: 550 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%' }}>
              ...
              >
                ...
                <div style={{
                  ...
                  <HiddenBalance balance={data.balance} pinCreated={data?.pin_created} />
                </div>
                ...
                <div style={{ padding: '1.5rem' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.25rem' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontWeight: 700, fontFamily: 'monospace', color: '#0A2540', fontSize: '1.05rem', letterSpacing: '0.5px' }}>
              ...
              {/* 2. QR Code Card */}
              <QRCodeCard accountNumber={data?.account_number} localIp={data?.local_ip} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', width: '100%' }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)' }}>
                  Virtual Ledger Access Card
                </div>
                <VirtualCard cardholderName={data.name} accountNumber={data.account_number} cardNumber={data.card_number} revealed={false} isCardActive={data.is_card_active !== false} />
              </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                  <div style={{ fontSize: '0.9rem', fontWeight: 800 }}>{data.name}</div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>Personal Wealth</div>
                </div>
              </div>
              <button 
                onClick={() => setShowDrawer(false)}
            ...
            <h4 style={{ fontSize: '0.85rem', fontWeight: 800, color: '#0A2540', margin: '0 0 1rem 0' }}>
              Security & Ledger Alerts
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', flex: 1 }}>
              {notifications.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '1rem 0', color: 'var(--text-secondary)', fontSize: '0.75rem' }}>
                ...
                notifications.map((notif) => {
                  ...
                  return (
                    <div key={notif.id} style={{
                      ...
                      padding: '0.6rem',
                      borderRadius: '10px',
                      backgroundColor: notif.read ? 'transparent' : 'rgba(10, 37, 64, 0.02)',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#0A2540', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{notif.title}</span>
                        </div>
                        <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', lineHeight: '1.3', marginTop: '0.1rem', wordBreak: 'break-word' }}>
                          {notif.message}
                        </div>
                      </div>
                    </div>
